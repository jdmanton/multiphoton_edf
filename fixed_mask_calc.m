...
%% Calculate pupil zone NAs based on physical mask dimensions
% Hamamatsu A12802-35-040 mask
mask_dias = [1.8, 2.5, 3.1, 3.6, 4.0]; % Diameter of mask zones in mm
num_steps = length(mask_dias);
mask_to_pupil_mag = 0.6 * 4; % Magnification from mask to galvos (0.6) to pupil (4)
nas = [0, mask_to_pupil_mag * mask_dias ./ (2 * (180 / 25))]; % Uses focal length of 25x Olympus lens
dof = wavelength / (refractive_index * (1 - cosd(asind(nas(2) / refractive_index))));


%% Set up simulation parameters
M = 150;                % PSF planes will have dimensions (2M + 1) by (2M + 1)
N = 1315;               % Scaling factor -- increase for smaller voxel size
